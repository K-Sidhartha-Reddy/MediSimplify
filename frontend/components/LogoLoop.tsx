...
const LogoLoop = memo(function LogoLoop({
  ...
  const rootClassName = useMemo(
    () =>
      [
        styles.logoloop,
        isVertical ? styles.logoloopVertical : "",
  ...
  const renderLogoItem = useCallback(
    (item: LogoItem, key: string) => {
      if (renderItem) {
        return (
          <li className={styles.logoloopItem} key={key} role="listitem">
            {renderItem(item, key)}
          </li>
        );
      }
      ...
      const content = isNodeItem ? (
        <span className={styles.logoloopNode} aria-hidden={!!item.href && !item.ariaLabel}>
          {item.node}
        </span>
      ) : (
        <img
  ...
  return (
    <div ref={containerRef} className={rootClassName} style={containerStyle} role="region" aria-label={ariaLabel}>
      <div className={styles.logoloopTrack} ref={trackRef} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
        {logoLists}
      </div>
    </div>
  );
