...
type BlurTextProps = {
  ...
  easing?: ((t: number) => number) | "linear" | "easeIn" | "easeOut" | "easeInOut" | number[] | "ease-out";
  onAnimationComplete?: () => void;
  stepDuration?: number;
};

const buildKeyframes = (from: AnimationState, steps: AnimationStep[]) => {
  const keys = new Set([
    ...Object.keys(from),
  ...
  keys.forEach((key) => {
    keyframes[key] = [
      (from[key] ?? 0) as string | number,
      ...steps.map((step) => (step[key] ?? from[key] ?? 0) as string | number)
    ];
  });
  ...
  return (
    <p ref={ref} className={className} style={{ display: "flex", flexWrap: "wrap" }}>
      {elements.map((segment, index) => {
        ...
        (spanTransition as { ease?: unknown }).ease = normalizedEase;
